...
let userID = '';
let wspath = '/';
//let proxyIP = '192.9.136.21';
//let proxyIP = '152.67.254.55';  // San 
//let proxyIP = '130.61.12.169'; // gre
//let proxyIP = '158.101.44.40'; // USA
let proxyIP = '150.136.241.40'; // USA
//let proxyIP = '103.200.112.108';//HK

//let proxyWeb='https://www.fan-2000.com';
let proxyWeb='https://lander.a-wsl.com';

let cfipv4=['173.245.48.0/20','103.21.244.0/22','103.22.200.0/22','103.31.4.0/22','141.101.64.0/18','108.162.192.0/18','190.93.240.0/20','188.114.96.0/20','197.234.240.0/22','198.41.128.0/17','162.158.0.0/15','104.16.0.0/13','104.24.0.0/14','172.64.0.0/13','131.0.72.0/22'];
let cfipv6=['2400:cb00::/32','2606:4700::/32','2803:f800::/32','2405:b500::/32','2405:8100::/32','2a06:98c0::/29','2c0f:f248::/32'];
...
//校验ip是不是在cidr中
const checkInCidr4Ipv6 = ip => cidr => {
	cidr = cidr.split('/');
...
function checkAddressPool4Ipv6(cidr,addresspools){
	...
	for(var i=0; i<addresses.length; i++)
		if(!checkInCidr4Ipv6(addresspools[i])(cidr))
			return false;
...
const isIp6InCidrs = (ip, cidrs) => cidrs.some(checkInCidr4Ipv6(ip));

function isCfIp(ip){
	return (ip.indexOf(':')>-1)?isIp6InCidrs(ip,cfipv6):isIp4InCidrs(ip,cfipv4);
}
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				switch (url.pathname) {
					//case '/':
						...
						//return new Response('Token and remote not found', { status: 200 });
					case `/showconfig`: {
					...
					default:
						return fetch(proxyWeb + url.pathname);
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, vlessResponseHeader, log,) {
	...
	const tcpSocket = await connectAndWrite(isCfIp(addressRemote)?proxyIP:addressRemote, portRemote);
...
function getVLESSConfig(userID, hostName) {
	const vlessMain = `vless://${userID}@${hostName}:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=`+ wspath +`%3Fed%3D2048#${hostName}`
	return `
################################################################
v2ray
---------------------------------------------------------------
